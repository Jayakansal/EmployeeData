...
salary=$(( $empHrs * $EMP_RATE_PER_HR )) 
echo "Salary: $salary"
#Calculate salary of an employee for a month using for loop(Use-case-5). #! /bin/bash 
IS_PART_TIME=1 
IS_FULL_TIME=2 
EMP_RATE_PER_HR=20 
NUM_WORKING_DAYS=20 
...
totalEmpHrs=0 
totalWorkingDays=0 
while(( $totalEmpHrs < $MAX_HRS_IN_MONTH && $totalWorkingDays < $NUM_WORKING_DAYS )) do 
 ((totalWorkingDays++)) 
 ...
 *) empHrs=0
 ...
 totalEmpHrs=$(( $totalEmpHrs + $empHrs ))  echo "Total Employee Hours: $totalEmpHrs" 
done 
totalSalary=$(( $totalEmpHrs * $EMP_RATE_PER_HR)) echo "Total Salary: $totalSalary"
IS_PART_TIME=1 
IS_FULL_TIME=2 
...
while(( $totalWorkHours < $MAX_HRS_IN_MONTH && $totalWorkingDays < $NUM_WORKING_DAYS )) do 
 ((totalWorkingDays++)) 
 #Calling function getWorkingHours by passing random number  workHours="$( getWorkingHours $(( RANDOM%3 )) )" 
 echo "Work hours from function getWorkingHours()::::::::::>>> $workHours "  totalWorkHours=$(( $totalWorkHours + $workHours )) 
 ...
 dayWiseWages[$totalWorkingDays]="$(calculateDailyWage $workHours)" done
